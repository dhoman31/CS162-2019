import java.util.Scanner;

public class TestEmployee{
    public static void main(String[] args) {
        Scanner sc = new Scanner(System.in);

        Employee emp1 = new Employee("Tom Daly", "Teacher", 45000f);
        Employee emp2 = new Employee("Mary Murphy", "Developer", 30000f);
        Employee emp3 = new Employee("Daniel Homan", "Software Engineer", 36000f);

        System.out.println("Employee 2 is called " + emp2.getName() + "\nThey are a " + emp2.getJobTitle() + "\nTheir Salary is " + emp2.getSalary() + "\nTheir Employee ID number is " + emp2.getEmployeeID());

        emp3.setName("Michael Higgins");
        emp3.setJobTitle("President");
        emp3.setSalary(1000);

        emp1.showEmp();
        emp2.showEmp();
        emp3.showEmp();

        System.out.println(emp1.toString());
    }
}